...
\item  The basic set of random geometrical objects are $\mathcal{R}$ valued random field $f(x)$ defined on $S^2$ and its excursion sets $A$ 
...
\frame {
  ...
  \begin{itemize} 
    ...
    \item The volume of a tube with radius $\rho$ bounding Maround M,  $Tube(M,\rho) = \{ t\in \mathcal{R}^N : dist(M,x)\leq\rho \}$ is given by (Steiner's formula) 
    
    %Lifschitz-Killing Curvatures (LKCs), a.k.a Minkowski Functionals (MFs), can be defined using a tube formula:
...
\item Lifschitz-Killing Curvatures (LKCs) $\mathcal{L}$ are related to Minkowski Functionals $\mathcal{M}_j$ 
\[
\mathcal{M_j} = (j! \omega_j)\mathcal{L_}{N-j} \hspace j=0,1,2...,N
\]
